...
for K in K_RANGE:
    ...
    for _ in range(TRIALS):
        ...
        # Raw cosine of accumulated drifted state to V (no cleanup, no unbinding)
        raw_drifted = S + noise_sum
        scores["No Intervention"].append(cosine(raw_drifted, V))

        # --- Method 2: Re-prompting (add V_safe back into drifted state) ---
        reprompt = S + noise_sum + V
        scores["Re-prompting"].append(cosine(reprompt, V))

        # --- Method 3: Similarity / RAG (best cosine among noise vectors to V) ---
        best_sim = max(cosine(n, V) for n in noise_vecs)
        scores["Similarity (RAG)"].append(best_sim)

        # --- Method 4: HIS (normalize noise, sign cleanup, unbind) ---
        # Normalization pins SNR at 0 dB (Section 3.2 of paper)
        noise_norm = np.linalg.norm(noise_sum)
        if noise_norm > 0:
            noise_normalized = noise_sum * (np.sqrt(D) / noise_norm)
        else:
            noise_normalized = noise_sum
        drifted = sign_cleanup(S + noise_normalized)
