...
# ---------------------------------------------------------------------------
_parser = argparse.ArgumentParser(description='Generate Experiment IV (drift) figures.')
_parser.add_argument('-a', action='store_true', help='Regenerate figure_9a (linear drift trajectories)')
_parser.add_argument('-b', action='store_true', help='Regenerate figure_9b (double-integrator ECBF)')
_parser.add_argument('-c', action='store_true', help='Regenerate figure_9c (L_f h distribution)')
_parser.add_argument('-d', action='store_true', help='Regenerate figure_9d (ECBF barrier values)')
...
_gen_combined = not _any_selected  # legacy combined figure only when regenerating all
...
def barrier_sphere(x):
    """h(x) = 1 - ||x||^2 (unit-ball safe set, quadratic barrier)."""
    return 1.0 - np.linalg.norm(x)**2
...
print(f"  Spectral margin rho(x) = sigma_max(A) * dt = {sigma_A * DT_DRIFT:.4f}")
...
for trial in range(N_TRIALS):
    ...
    q_goal = 1.5 * q_goal / np.linalg.norm(q_goal)  # outside safe set
...
q_goal_test = rng.randn(DIM_HALF); q_goal_test = 1.5 * q_goal_test / np.linalg.norm(q_goal_test)
...
# Helper: which panels to generate
print(f"\nPanels to generate: "
...
# ============================================================================
if _gen_combined:
    print("\nGenerating figure_9.png (combined) ...")
    ...
    fig.suptitle('Experiment IV: Safety Under Non-Trivial Drift Dynamics ($\\mathbb{R}^{128}$)',
                 fontsize=15, fontweight='bold', y=1.01)
    plt.tight_layout()
    plt.savefig(os.path.join(script_dir, 'figure_9.png'), dpi=300, bbox_inches='tight',
                facecolor='white', edgecolor='none')
    plt.close()
    print("[OK] Saved figure_9.png (combined, legacy)")

# ============================================================================
# INDIVIDUAL PANEL FIGURES (for split paper layout)
# ============================================================================

# --- Figure 9a: Linear Drift Trajectories ---
if _gen_a:
    ...
    ax.set_xlabel('Time (s)', fontsize=12)
    ax.set_ylabel('$\\|x\\|$', fontsize=12)
    ax.set_title('Linear Drift Dynamics: $\\dot{x} = Ax + u$\n'
                 f'$\\sigma_{{\\max}}(A)={sigma_A:.3f}$,  '
                 f'$L_f h$ mean $= {mean_Lfh:.3f}$ (drift toward boundary)',
                 fontsize=12, fontweight='bold')
    ax.legend(loc='lower right', fontsize=9, framealpha=0.9)
    ...
    ax.text(0.03, 0.97,
            ...
            bbox=dict(boxstyle='round,pad=0.3', fc='white', alpha=0.85))
    plt.tight_layout()
    fig_a.savefig(os.path.join(script_dir, 'figure_9a.png'), dpi=300, bbox_inches='tight',
                  facecolor='white', edgecolor='none')
    plt.close(fig_a)
    print("[OK] Saved figure_9a.png")

# --- Figure 9b: Double-Integrator Trajectories ---
if _gen_b:
    ...
    ax.set_xlabel('Time (s)', fontsize=12)
    ax.set_ylabel('$\\|q\\|$ (position norm)', fontsize=12)
    ax.set_title('Double-Integrator: $[\\dot{q};\\dot{v}] = [v; u]$ with ECBF\n'
                 f'$\\alpha_1={ALPHA1}$, $\\alpha_2={ALPHA2}$  '
                 f'(64 pos + 64 vel = $\\mathbb{{R}}^{{128}}$)',
                 fontsize=12, fontweight='bold')
    ax.legend(loc='lower right', fontsize=9, framealpha=0.9)
    ...
    ax.text(0.03, 0.97,
            ...
            bbox=dict(boxstyle='round,pad=0.3', fc='white', alpha=0.85))
    plt.tight_layout()
    fig_b.savefig(os.path.join(script_dir, 'figure_9b.png'), dpi=300, bbox_inches='tight',
                  facecolor='white', edgecolor='none')
    plt.close(fig_b)
    print("[OK] Saved figure_9b.png")

# --- Figure 9c: L_f h Distribution ---
if _gen_c:
    ...
    ax.set_xlabel('$L_f h(x) = \\nabla h \\cdot f(x)$', fontsize=12)
    ax.set_ylabel('Count', fontsize=12)
    ax.set_title('Distribution of Drift Lie Derivative $L_f h$\n'
                 '(negative values = drift pushes toward boundary)',
                 fontsize=12, fontweight='bold')
    ax.legend(fontsize=10, framealpha=0.9)
    ax.grid(True, alpha=0.2)
    ax.text(0.97, 0.97,
            ...
            bbox=dict(boxstyle='round,pad=0.3', fc='#f0e6ff', alpha=0.85))
    plt.tight_layout()
    fig_c.savefig(os.path.join(script_dir, 'figure_9c.png'), dpi=300, bbox_inches='tight',
                  facecolor='white', edgecolor='none')
    plt.close(fig_c)
    print("[OK] Saved figure_9c.png")

# --- Figure 9d: ECBF Barrier Values ---
if _gen_d:
    ...
    ax.set_xlabel('Time (s)', fontsize=12)
    ax.set_ylabel('Barrier Value', fontsize=12)
    ax.set_title('ECBF Barrier Functions (Representative Trial)\n'
                 '$\\psi_0 \\geq 0$ (safety) and $\\psi_1 \\geq 0$ (ECBF condition)',
                 fontsize=12, fontweight='bold')
    ax.legend(loc='upper right', fontsize=9, framealpha=0.9)
    ax.grid(True, alpha=0.2)
    ...
    ax.text(0.03, 0.03,
            ...
            bbox=dict(boxstyle='round,pad=0.3', fc='white', alpha=0.85))
    plt.tight_layout()
    fig_d.savefig(os.path.join(script_dir, 'figure_9d.png'), dpi=300, bbox_inches='tight',
                  facecolor='white', edgecolor='none')
    plt.close(fig_d)
    print("[OK] Saved figure_9d.png")
