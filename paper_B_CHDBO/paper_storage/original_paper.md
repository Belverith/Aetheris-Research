...
\begin{abstract}
Verifying safety constraints in high-dimensional state spaces is computationally intractable for deterministic methods: grid-based Hamilton-Jacobi reachability scales as $O((1/\eta)^n)$, becoming infeasible for $n > 5$. We introduce \textit{Constrained High-Dimensional Barrier Optimization} (CHDBO), a framework that replaces deterministic enumeration with probabilistic certification. Using Monte Carlo Barrier Certificates bounded by Lipschitz continuity, CHDBO achieves a sample count---$N = O(\epsilon^{-2}\ln\delta^{-1})$---that is independent of the state-space dimension $n$ for any fixed Lipschitz-continuous barrier, with total cost $O(Nn)$, enabling safety certification at $n \geq 128$. A projected-gradient utility maximizer ensures asymptotic convergence to constrained optima without violating the safety envelope. To detect rare catastrophic modes invisible to passive sampling, we introduce \textit{Active Adversarial Safety Verification} (AASV): momentum-accelerated adversarial search with Hutchinson-bounded spectral margins and orthogonal prototype memory, achieving $O(n)$ per-step cost when automatic-differentiation-based Jacobian-vector products are employed (the accompanying figure-generation code uses $O(n^2)$ dense matrix operations for reproducibility). For multi-modal barrier landscapes, AASV requires structural gradient access via Winner-Take-All decomposition---a barrier design guideline, not a general-purpose discovery tool for arbitrary unknown failure modes (Section~\ref{sec:wta_limitation}). Experiments across fourteen configurations---spanning chaotic nonlinear dynamics, multi-modal barriers in $\mathbb{R}^{128}$, scalability to $n = 1024$, and GPT-2 hidden-state dynamics ($\mathbb{R}^{768}$) with a data-learned barrier on a production toxicity benchmark---confirm that CHDBO maintains safety while preserving utility. Three additional experiments provide honest failure-mode characterization: learned neural network barriers enable AASV detection without structural oracles (Experiment~XV), bounded actuation degrades gracefully with a quantified phase transition (Experiment~XVI), and systematic negative-result analysis identifies the regimes where guarantees break down (Experiment~XVII).
\end{abstract}
...
Modern autonomous agents do not merely process text; they operate in high-dimensional state spaces, mapping inputs to actions in continuous environments \cite{ames2019}. Whether controlling a swarm of drones ($n>100$) \cite{brunke2022} or navigating the semantic embedding space of a multi-modal reasoning engine ($n=12,288$ \cite{brown2020, bengio2013}) or a visual transformer \cite{radford2021}, the agent must constantly optimize for utility while adhering to strict safety constraints (e.g., ``Do not delete root files,'' ``Do not collide'').
...
\begin{itemize}
    ...
    \item \textbf{Active Adversarial Safety Verification (AASV):} We introduce an adversarial verification module that replaces passive Monte Carlo sampling with gradient-based adversarial search, Hutchinson-bounded spectral margins \cite{hutchinson1990}, and holographic orthogonal prototype memory, converting the probabilistic ``Black Swan'' vulnerability into a bounded adversarial robustness guarantee while preserving $O(n)$ computational scaling (with AD-based JVPs; current figure-generation code uses $O(n^2)$ dense matrix operations). For multi-modal barrier landscapes, this guarantee requires structural gradient access via Winner-Take-All (WTA) decomposition---a \textit{barrier design guideline} requiring practitioners to structure the barrier so that per-component gradients are available (e.g., sum-of-Gaussian wells or multi-head neural architectures \cite{dawson2023}), rather than a general-purpose discovery tool for arbitrary unknown failure modes. For barriers with a single dominant failure mode, standard (black-box or oracle) gradients suffice (Section~\ref{sec:wta_limitation}, Experiment~VI).
\end{itemize}
...
\textbf{Hamilton-Jacobi Reachability.}
The gold standard for deterministic safety verification is H-J reachability analysis \cite{mitchell2005}, which computes the backward reachable set via viscosity solutions of the H-J PDE. While providing the strongest possible guarantees, its $O((1/\eta)^n)$ grid complexity limits practical application to $n \leq 5$. Recent work on decomposition-based \cite{chen2018hj} and learning-based approximations---notably DeepReach \cite{bansal2021}, which parameterizes the value function with a neural network and solves the H-J PDE via self-supervised sinusoidal representations, achieving approximate reachability analysis up to $n \approx 10$---aims to extend reachability to moderate dimensions, but the fundamental exponential barrier remains for exact computation and neural approximations lack formal verification guarantees for $n \gg 10$. Our probabilistic relaxation sidesteps this barrier entirely.
...
\begin{enumerate}
    ...
    \item \textbf{Scalability:} The computation time for calculating $u$ is $O(n)$ relative to dimension (linear scaling) when automatic-differentiation-based Jacobian-vector products (JVPs/VJPs) are employed; current figure-generation code uses $O(n^2)$ dense matrix operations for reproducibility (see Section~3.5.2, Figure~\ref{fig:computational_scaling}). This enables real-time operation for $n \gg 10$.
\end{enumerate}
\end{problem}
...
\begin{enumerate}
    ...
    \item[\textbf{A3.}] \textbf{Relative Degree One:} The barrier $h(x)$ has relative degree one with respect to the control input $u$, i.e., $L_g h(x) \neq 0$ on $\partial\mathcal{S}$. For higher relative degree, an Exponential CBF (ECBF) extension is employed \cite{xiao2019} (see Remark below and empirical validation in Section~5.5).

\textbf{Remark (Exponential CBF for Higher Relative Degree).} When $h(x)$ has relative degree $r \geq 2$ (i.e., $L_g h = L_g L_f h = \cdots = L_g L_f^{r-2} h = 0$), the standard CBF condition does not directly constrain $u$. The Exponential CBF approach \cite{xiao2019} constructs a sequence of auxiliary functions $\psi_0 = h$, $\psi_i(x) = \dot{\psi}_{i-1}(x) + \alpha_i \psi_{i-1}(x)$ for $i = 1, \ldots, r{-}1$ (with $\alpha_i > 0$), until $\psi_{r-1}$ has relative degree one and the standard CBF-QP applies. This reduces the higher-order constraint to a single affine inequality in $u$, preserving the $O(n)$ computational framework. The pole-placement parameters $\alpha_i$ govern the convergence rate: larger values yield faster barrier recovery but require greater control authority.
    ...
    \item[\textbf{A7.}] \textbf{Continuous Relaxation Validity (Semantic Systems):} When applying CHDBO to discrete semantic systems (e.g., Transformer embedding spaces), the dynamics are modeled as a continuous ODE $\dot{x} = f_\theta(x,t)$ via the Neural ODE paradigm \cite{chen2018, kidger2022, geshkovski2024}. A related interpretation arises from Deep Equilibrium Models \cite{bai2019}, which model hidden layers as fixed points of an implicit map rather than explicit sequential updates; both paradigms support the continuous-time view of transformer dynamics underlying Assumption~A7. This is a modeling assumption whose fidelity to the true discrete dynamics is governed by the single-step Euler local truncation error $O(\Delta t^2 \cdot L_{\nabla f})$, which is absorbed into $\epsilon_{\text{model}}$. For architectures where the Lipschitz constant $L_f$ may be potentially very large (e.g., softmax attention with peaked distributions \cite{virmaux2018, kim2021}), the framework becomes conservative (the feasible set shrinks), which is the appropriate fail-safe behavior. Practitioners must validate the tightness of the discretization bound empirically for their specific architecture.
    \item[\textbf{A8.}] \textbf{Controllability Margin:} There exists a constant $c > 0$ such that $\inf_{x \in \partial\mathcal{S}} \|L_g h(x)\| / (|L_f h(x)| + 1) > c$, ensuring the control input retains sufficient authority to counteract drift at the safety boundary. When this margin degrades (e.g., due to large $L_f$ from softmax attention saturation), the CBF-QP may become infeasible; the framework responds by engaging the safe backup trajectory (Section~3.5.4). For the synthetic experiments (Experiments I--XIII), this assumption is trivially satisfied since $g(x) = I$. For Experiment~XIV (GPT-2), the controllability margin is verified empirically: the pre-intervention margin $w \cdot f(x_{\text{bnd}})$ exceeds the minimum required actuation by a factor of $\sim 20\times$ (Section~\ref{sec:exp_gpt2}).
\end{enumerate}
...
As established in Section~2, the computational cost of exact verification scales as $O((1/\eta)^n)$. To contextualize this: verifying a standard 3-link robotic arm ($n=6$) on a coarse grid requires $10^{12}$ evaluations. Verifying a swarm of 50 drones or a semantic embedding space ($n \geq 128$) requires $10^{200}$ evaluations, rendering deterministic grid-based verification physically intractable regardless of available computing power. Two classical results from high-dimensional geometry provide supporting intuition. First, the \textit{Concentration of Measure} phenomenon (L\'{e}vy's lemma; see \cite{vershynin2018}, Ch.~5) states that any Lipschitz-continuous function on $S^{n-1}$ concentrates tightly around its median as $n \to \infty$, implying that local barrier statistics estimated from random boundary samples are representative of the global boundary behavior. Second, the Johnson-Lindenstrauss lemma \cite{johnson1984, dasgupta2003} guarantees that pairwise distances between boundary samples are approximately preserved under random low-dimensional projections, providing geometric evidence that random sampling does not systematically distort the proximity structure relevant to safety verification. Neither result directly guarantees preservation of barrier function values or set membership; together, however, they motivate the hypothesis---validated empirically in our experiments---that sparse random boundary sampling can capture the essential safety geometry without exhaustive enumeration.

To solve this, we shift our verification criterion from Worst-Case Invariance (is there any point $x$ where safety fails?) to PAC-Style Invariance \cite{valiant1984} (is the probability of encountering a failure state bounded by $\epsilon$?). We employ the two-sided Chernoff--Hoeffding bound for Bernoulli random variables ($P[|\hat{p} - p| > \varepsilon] \leq 2e^{-2N\varepsilon^2}$) to bound the estimation error $\epsilon$ \cite{hoeffding1963}. This approach aligns with modern uncertainty quantification methods, such as Conformal Prediction \cite{angelopoulos2021, lindemann2023}, and established randomized control frameworks \cite{tempo2012, calafiore2006}, effectively constructing a probabilistic envelope around the system dynamics rather than a rigid boundary.
...
\begin{figure}[!htbp]
    ...
    \caption{\texorpdfstring{\textbf{The `Hollow Ball': Concentration of Measure in High Dimensions.}}{The 'Hollow Ball': Concentration of Measure in High Dimensions.} In low dimensions ($n=3$), volume is spread broadly across all radii. As dimensionality increases through $n=10$ and $n=50$, the density progressively sharpens toward the boundary. In high-dimensional semantic spaces ($n=128$), probability mass concentrates almost exclusively in a thin `Active Safety Shell' near the boundary ($r > 0.95$, where ${\approx}\,99.9\%$ of the volume resides). This validates the CHDBO sampling strategy: verifying the boundary effectively verifies the entire volume.}
...
\textbf{Remark (Consolidated Dimension-Independence Caveats).}
While the \textit{sample count} $N$ required by the Hoeffding bound (Equation~\ref{eq:hoeffding}) is independent of the state-space dimension $n$, three aspects of the verification pipeline do carry implicit or explicit dimensional dependence, and we consolidate them here for transparency:
\begin{enumerate}
...
\end{enumerate}
These caveats do not undermine the central $O(1)$ sample complexity claim---they clarify the precise sense in which it holds and identify the conditions under which practitioners should exercise additional caution.
...
\textbf{Remark (Horizon Limitation).} The union bound becomes vacuous ($\geq 1$) when $T \geq 1/\epsilon$. For $\epsilon = 10^{-6}$, this occurs at $T = 10^6$ steps ($\approx 2.8$ hours at 10\,ms/step). For longer deployment horizons, three mitigations apply: (1)~the AASV module provides per-step adversarial verification independent of horizon length, catching violations the MCBC may miss; (2)~periodic re-certification (re-running Algorithm~\ref{alg:verify}) resets the union bound counter, yielding a failure probability of at most $T_{\text{window}} \cdot \epsilon$ per certification window; and (3)~for stochastic systems, tighter trajectory-level bounds can be obtained via supermartingale barrier certificates \cite{santoyo2021, clark2021}, which exploit the barrier's martingale structure to avoid the linear accumulation of the union bound. Integrating supermartingale arguments with the CHDBO pipeline is an important direction for future work; see Figure~\ref{fig:union_bound_horizon} for a visualization of how trajectory-level safety degrades with horizon length under the union bound versus re-certification strategies.
...
We therefore use a \textbf{Barbalat-type argument} \cite{barbalat1959, khalil2002} instead of LaSalle's invariance principle \cite{lasalle1960}. Since $V$ is bounded below on compact $\mathcal{S}$ and the trajectory $x(t)$ remains in $\mathcal{S}$ by forward invariance, $V(x(t))$ is a bounded function of time. We now establish that $\int_0^T \|\nabla U(x(t))\|^2\, dt$ is bounded for all $T$. Decompose the time axis into \textit{interior phases} (where $h(x(t)) > 0$ and the CBF constraint is inactive) and \textit{boundary phases} (where the constraint is active). During interior phases, $\dot{V} = -\|\nabla U\|^2 \leq 0$, so $V$ is non-increasing and $\int \|\nabla U\|^2\, dt \leq V(x(0)) - \inf_\mathcal{S} V < \infty$. During boundary phases, the trajectory evolves on the codimension-1 manifold $\partial\mathcal{S}$. The projected dynamics on $\partial\mathcal{S}$ satisfy $\dot{x} = u^* = \nabla U + \lambda^* \nabla h$, where $\lambda^*$ is determined by $\dot{h} = -\gamma h = 0$. We define the \textit{projected Lyapunov function} $W(x) = V(x)|_{\partial\mathcal{S}}$, whose time derivative along the boundary is $\dot{W} = -\|\nabla U\|^2 - \lambda^* \nabla U \cdot \nabla h = -\|\text{Proj}_{\text{Null}(\nabla h)} \nabla U\|^2 \leq 0$, where the last equality uses the geometric decomposition of the projected gradient (Section~4.2). Thus $V$ is non-increasing during boundary phases as well, provided we project onto the tangent space. The total integral $\int_0^\infty \|\text{Proj}_{\text{Null}(\nabla h)} \nabla U\|^2\, dt$ is bounded by $V(x(0)) - \inf_\mathcal{S} V$.

We note that $V$ may transiently increase during boundary phases when $\lambda^* \nabla U \cdot \nabla h > \|\nabla U\|^2$ (i.e., the drift-compensation term dominates), but $V$ remains bounded on the compact forward-invariant set $\mathcal{S}$, so the total ``positive excursion'' is finite and does not invalidate the subsequent Barbalat argument.

\textbf{Remark (Assumption A3 Requirement).} The convergence argument above requires that the CBF-QP remains feasible (Assumption A4) and that the control authority is sufficient to enforce $\dot{h} \geq -\gamma h$ while simultaneously reducing $V$ on average. When the drift term $f(x)$ is adversarial and dominates the available control authority (i.e., $\sup_u L_g h \cdot u < |L_f h|$), the CBF-QP may become infeasible or the KKT multiplier $\lambda^*$ may grow without bound, preventing utility convergence. In such regimes, the system converges to the best achievable point within the constrained critical set, which may not coincide with $\text{argmax}_{\mathcal{S}} U$. This limitation is discussed further in the General Control-Affine Dynamics remark below.

\textbf{Extension to General Control-Affine Dynamics (Proof Sketch).} The single-integrator Lyapunov analysis above extends to $\dot{x} = f(x) + g(x)u$ under one additional assumption: the control authority $g(x)$ must be sufficient to compensate the drift $f(x)$, i.e., $L_g V(x) \neq 0$ whenever $L_f V(x) > 0$ (the drift would increase $V$). Under this assumption, the nominal control $u_{\text{nom}}(x)$ can be designed to achieve $\dot{V} = L_f V + L_g V \cdot u_{\text{nom}} \leq 0$ in the interior, and the Barbalat argument below applies with $\dot{V} = -\nabla U^T (f(x) + g(x) u^*)$ replacing $\dot{V} = -\|\nabla U\|^2$. The proof as presented is \textit{complete} for single-integrator systems (Experiments I--IV, VI--VII, XI--XIV); for general drift dynamics (Experiments V, VIII--X), it is a proof sketch whose rigor depends on the controllability margin $\inf_{x \in \partial\mathcal{S}} \|L_g h(x)\| / |L_f h(x)|$ remaining bounded away from zero.

To apply Barbalat's Lemma, we verify that $\ddot{V}$ is bounded: since $\dot{V} = -\nabla U^T (f + gu^*)$ and both $U$, $h$ are $C^1$ (Assumptions A2, A6), and $u^*$ is a continuous function of $x$ (as the KKT solution of the convex QP), $\dot{V}$ is continuous in $x$. On the compact set $\mathcal{S}$, $\dot{V}$ is therefore Lipschitz in $x$, implying $|\ddot{V}| \leq L_{\dot{V}} \|\dot{x}\| \leq L_{\dot{V}} \cdot M$ where $M = \sup_{x \in \mathcal{S}, u \in \mathcal{U}} \|f(x) + g(x)u\|$ is finite by compactness (Assumption A4). With $\ddot{V}$ bounded, Barbalat's Lemma yields $\|\text{Proj}_{\text{Null}(\nabla h)} \nabla U(x(t))\|^2 \to 0$. Since $\mathcal{S}$ is compact and forward-invariant, the trajectory $\{x(t)\}_{t \geq 0}$ is pre-compact, so its $\omega$-limit set $\Omega \subset \mathcal{S}$ is non-empty, compact, and connected \cite{khalil2002}. Because the projected gradient vanishes on $\Omega$, every point of $\Omega$ satisfies the KKT conditions $\nabla U(x) + \lambda \nabla h(x) = 0$ for some $\lambda \geq 0$, $\lambda h(x) = 0$---precisely the first-order optimality conditions of $\max_\mathcal{S} U$.

\textbf{Singleton Convergence.} If, additionally, $U$ is real-analytic on $\mathcal{S}$, then the constrained Lagrangian $\mathcal{L}(x, \lambda) = -U(x) + \lambda h(x)$ satisfies the {\L}ojasiewicz gradient inequality \cite{lojasiewicz1963}: there exist $C > 0$ and $\theta \in [1/2, 1)$ such that $\|\nabla_x \mathcal{L}\| \geq C |V(x) - V(x^*)|^\theta$ near any critical point $x^*$. Combined with the bounded $\ddot{V}$ established above, this implies finite arc-length of the trajectory $\int_0^\infty \|\dot{x}\|\, dt < \infty$ \cite{absil2005}, and hence convergence to a \textit{single} KKT point $x^*_\mathcal{S} \in \Omega^*$, rather than oscillation within the KKT set. We note that the real-analyticity requirement (which strengthens Assumption A6) is satisfied by polynomial and rational utility functions but not by general $C^\infty$ functions; for merely smooth $U$, convergence to the KKT \textit{set} is guaranteed but convergence to a single point requires additional structural assumptions (e.g., isolation of critical points).
...
\textbf{Remark (Convergence Rate).} The Barbalat-based argument above establishes \textit{asymptotic} convergence ($\|\text{Proj}_{\text{Null}(\nabla h)} \nabla U\| \to 0$) but does not provide a convergence rate. For the single-integrator case with concave $U$ and convex $\mathcal{S}$, projected gradient descent achieves $O(1/t)$ convergence of the utility suboptimality gap $U(x^*_{\mathcal{S}}) - U(x(t))$ \cite{nesterov2004}; if $U$ additionally satisfies the Polyak-{\L}ojasiewicz (PL) inequality on $\mathcal{S}$, the convergence becomes linear (exponential): $U(x^*_{\mathcal{S}}) - U(x(t)) \leq (1-\mu/L)^t \cdot [U(x^*_{\mathcal{S}}) - U(x(0))]$, where $\mu$ is the PL constant and $L$ is the smoothness constant. For the {\L}ojasiewicz-based singleton convergence, the rate depends on the {\L}ojasiewicz exponent $\theta$: $O(e^{-ct})$ when $\theta = 1/2$ (the analytic case), and $O(t^{-\theta/(1-2\theta)})$ when $\theta \in (1/2, 1)$. Quantifying these rates empirically and establishing dimension-dependence of $\mu$ and $L$ for specific barrier geometries is left for future work.
\end{proof}
...
\textbf{Remark (Barrier Function Forms).} Two barrier function forms are used throughout the experiments: (1)~the \textit{linear barrier} $h(x) = 1 - \|x\|$ (Experiments I--IV, VI--VII), which defines a unit-ball safe set and whose gradient $\nabla h = -x/\|x\|$ is well-defined away from the origin; and (2)~the \textit{quadratic barrier} $h(x) = 1 - \|x\|^2$ (Experiments V, VIII--X), which also defines a unit-ball safe set but yields a CBF constraint that is \textit{affine} in $u$ (since $\nabla h = -2x$ is linear in $x$), ensuring the CBF-QP reduces to a single linear inequality and admits the closed-form solution of Section~4.2. The quadratic form is preferred whenever drift dynamics are present ($f(x) \neq 0$) because the resulting Lie derivative $L_f h = -2 x^T f(x)$ is linear in $x$, simplifying the QP. Both forms define the same safe set $\mathcal{S} = \{x : \|x\| \leq 1\}$; the choice affects only the CBF enforcement dynamics near the boundary, not the safety guarantee.
...
\begin{itemize}
    \item \textbf{Setup:} The agent possesses a ``bias'' vector forcing it rightward ($+x$), simulating a corrupted internal goal or ``mutational meltdown'' (the irreversible accumulation of deleterious drift once error rate exceeds a critical threshold \cite{lynch1995}).
    \item \textbf{Standard Agent (Unsafe, red):} Lacking verification, the agent follows the noise and bias directly into the forbidden zone, breaching the safety constraint $x=0.8$.
    \item \textbf{Orthogonal Verifier (Safe, green):} The verifier intercepts the velocity vector at every time step. As the agent approaches the theoretical bound, it projects the velocity onto the tangent of the safety manifold.
\end{itemize}
...
\textbf{Implementation Note.} The figure-generation code implements safety via direct tangent-plane projection: when the next state would breach $x = 0.8$, the normal component of velocity is zeroed. For single-integrator dynamics ($\dot{x} = u$) with a linear barrier $h(x) = 0.8 - x_1$, this tangential projection is \textit{algebraically equivalent} to the CBF-QP closed-form solution (Equation~\ref{eq:cbfqp}) in the limit $\gamma \to \infty$: the CBF-QP projects $u_{\text{nom}}$ onto the half-space $\{u : \nabla h \cdot u \geq -\gamma h\}$, which, when $h \to 0^+$ at the boundary, reduces to removing the component of $u_{\text{nom}}$ along $-\nabla h$. The qualitative result---an impenetrable geometric barrier at $x = 0.8$---is therefore identical under both formulations. See \texttt{prove\_safety.py} in the supplementary code.

\subsection{Experiment II: High-Dimensional Survival \texorpdfstring{($\mathbb{R}^{128}$)}{(R\textasciicircum 128)}}
To prove the scalability of CHDBO, we extended the simulation to 128 dimensions, mimicking the embedding space of a VSA-based semantic agent. The agent was subjected to a ``Gradient Attack'' \cite{madry2018} where the goal vector was placed explicitly outside the safe hypersphere ($R > 1.0$).

\begin{itemize}
    \item \textbf{Method:} We utilized the Monte Carlo Barrier Certificate (MCBC) engine to generate 50 adversarial trials of 100-step random walks under intense gradient pressure ($\text{Attack Strength} = 0.05$).
...
\begin{figure}[htbp]
    ...
    \caption{\texorpdfstring{\textbf{High-Dimensional Orthogonal Verification ($D=128$) Adversarial Gradient Attack Simulation.}}{High-Dimensional Orthogonal Verification (D=128) Adversarial Gradient Attack Simulation.} Standard agents (red) immediately breach the safe manifold boundary, drifting into the unsafe region. Verified agents (green) cluster tightly on the surface of the hypersphere. The PCA projection illustrates that despite the high dimensionality, the projection logic maintains the agent on the ``skin'' of the safe manifold. (Note: the 2D PCA projection is a visualization aid that captures the two directions of greatest variance; the safety guarantee is established by the barrier condition $h(x) \geq 0$ in the full $\mathbb{R}^{128}$, not by the projection.)}
    \label{fig:high_dim_survival}
\end{figure}

\textbf{Result:}
\begin{itemize}
    \item \textbf{Standard Agent (Breach, red):} Immediately breached the unit hypersphere, drifting into the unsafe region. This mirrors the ``Logic Regression'' observed in unverified LLM agents \cite{scrivens2026, wei2023}.
    \item \textbf{Verified Agent (Safe, green):} Clustered tightly on the surface of the hypersphere. The PCA projection illustrates that despite the high dimensionality, the projection logic maintains the agent on the ``skin'' of the safe manifold without requiring a grid-based computation.
\end{itemize}
...
\textbf{Implementation Note.} As with Experiment~I, the figure-generation code uses tangent-plane projection (removing the radial velocity component) rather than explicitly solving the CBF-QP. For a spherical barrier $h(x) = 1 - \|x\|$ and single-integrator dynamics, the two formulations produce identical boundary behavior. A hard re-normalization clamp ($x \leftarrow x/\|x\|$ when $\|x\| > 1$) provides an additional safety net against floating-point drift; this acts as an instantaneous reset and is strictly more conservative than the CBF-QP alone. See \texttt{prove\_safety\_high\_dim.py} in the supplementary code.
...
\]
where $\hat{t}$ is the unit tangent direction and $\hat{r}$ is the inward radial (unsafe) direction, so that $\alpha = \sin\theta$ where $\theta$ is the angle between the intent and the safety boundary. This ensures $\|v\| = 1$ (before noise) and produces the Pythagorean geometric decay $\sqrt{1-\alpha^2}$ in Semantic Preservation after orthogonal projection.
...
\begin{itemize}
    ...
    \item \textbf{High Threat ($\alpha > 0.8$):} Similarity drops precipitously as the Verifier aggressively filters the vector to prevent a breach. The horizontal reference line at $\cos\theta = 0.7$ (``Acceptable Utility Threshold'') is crossed near $\alpha \approx 0.55$, indicating that beyond a 50/50 intent split the agent's original action is substantially reshaped.
    \item \textbf{Curve:} The simulation data closely tracks the theoretical geometric decay $\sqrt{1-\alpha^2}$ characteristic of orthogonal projections in high-dimensional Euclidean space \cite{vershynin2018, blum2020}, with a systematic offset below the noiseless curve. This offset is expected: the simulation includes a 10\% isotropic noise component (modeling sensor or embedding noise), which perturbs the velocity away from the pure radial--tangential decomposition assumed by the $\sqrt{1-\alpha^2}$ derivation. Notably, at high adversarial intensity ($\alpha \to 1$), the empirical similarity floors near $0.1$--$0.2$ rather than reaching zero; this occurs because the noise's tangential components survive projection even when the intended radial signal is fully removed. This residual utility floor is a desirable property---the system gracefully degrades rather than collapsing to zero output under maximum adversarial pressure.
\end{itemize}

This confirms that the implementation matches the theoretical prediction of Section~4.2: the orthogonal projection yields the geometric decay $\sqrt{1-\alpha^2}$ as an upper bound on achievable semantic preservation, and the CBF-QP filter preserves the maximum utility component orthogonal to the constraint gradient. The agent does not stop at the boundary; it maximizes the component of the utility vector that is orthogonal to the constraint gradient. This ensures the agent maintains tangential motion along $\partial\mathcal{S}$, achieving the maximum possible utility permitted by the safety constraints. (The figure-generation code uses direct tangent-plane projection, which is equivalent to the CBF-QP for single-integrator dynamics with a linear barrier; see Implementation Notes in Experiments~I and~II.)
...
\textbf{Results:} Both configurations achieve \textbf{0/50 safety violations}. Under linear drift, the mean Lie derivative contribution is $L_f h = -0.26$ (adverse---the drift pushes the system toward the boundary), yet the CBF-QP compensates via the $L_g h \cdot u$ control term, maintaining $\|x\| < 1$ at all times. Under double-integrator dynamics, the ECBF maintains $\psi_0 \geq 0$ (safety) and $\psi_1 \geq 0$ (the cascaded ECBF auxiliary barrier) throughout all trials, with $\max \|q\| = 0.998$. Both configurations remain under 0.01\,ms per step via the closed-form QP solution, confirming that the $O(n)$ computational budget is preserved even with non-zero drift. These results bridge the gap identified in the Limitations (Section~5.10): the CBF-QP framework handles non-trivial $f(x) \neq 0$ dynamics without modification to the core algorithm.
...
\end{equation}
with standard parameters $(\sigma, \rho, \beta) = (10, 28, 8/3)$ and a scaling factor $1/8$ to reduce the attractor scale to be comparable to the unit ball, with the CBF-QP enforcing confinement. This rescaling reduces the effective Lyapunov exponent by the same factor (i.e., the rescaled system has $\lambda_{\max} \approx 0.9/8 \approx 0.11$), so the dynamics remain meaningfully chaotic with substantially stronger nonlinear forcing than the previously reported $1/40$ scaling. The purpose of this experiment is to demonstrate CBF-QP enforcement under chaotic nonlinear coupling; practitioners applying CHDBO to unscaled chaotic systems should expect significantly higher control effort and CBF activation frequency. Crucially, adjacent triplets are coupled via nearest-neighbor diffusion with coupling strength $\kappa = 0.5$:
\begin{equation}
\dot{x}_{3i} \mathrel{+}= \kappa(x_{3(i{+}1)} - x_{3i}), \qquad \dot{x}_{3i+2} \mathrel{+}= \kappa(x_{3(i{-}1)+2} - x_{3i+2})
\end{equation}
...
\textbf{Results:} Over 50 adversarial trials (2{,}000 steps each), the CBF-QP maintains \textbf{0/50 safety violations} despite mean drift magnitude $\|f(x)\| \approx 11.2$ and strongly negative $L_f h$. The $1/8$ scaling factor (compared to $1/40$ in an earlier version) produces substantially stronger nonlinear forcing while the RK4 integrator with $\Delta t = 0.001$ ensures accurate trajectory integration. The quadratic CBF-QP handles genuinely nonlinear cubic coupling terms \textit{and} inter-triplet diffusive coupling without modification. The nearest-neighbor coupling ensures that perturbations propagate across all 42 triplets, producing spatiotemporally chaotic dynamics rather than 42 independent 3D attractors. This experiment extends the linear-drift validation of Experiment~V to a coupled chaotic nonlinear regime, confirming that the continuous-relaxation safety theory (Theorem~\ref{thm:topological_safety}) holds in practice for the most challenging class of drift dynamics tested.

\subsection{Experiment IX: Scalability Beyond \texorpdfstring{$n = 128$}{n = 128}}
All previous experiments operate at $n = 128$. To validate that CHDBO scales practically to higher dimensions, we run the full pipeline---linear drift ($\sigma_{\max}(A) \approx 1.04$), CBF-QP enforcement with integrated Hutchinson spectral estimation ($k = 5$ probes), and adaptive safety margin $\rho(x) = \tilde{\sigma}_{\max} \cdot \Delta t$---at $n \in \{128, 512, 1024\}$ with 20 trials $\times$ 300 steps per dimension. Critically, the Hutchinson spectral estimate is \textit{actively fed into the CBF-QP constraint} at each step via Equation~\ref{eq:robust_barrier}, tightening the barrier condition by $\rho(x)$ to account for drift volatility. We deliberately use $k = 5$ probes (below the $m \geq 20$ recommendation of Section~3.5.2) to demonstrate that the Frobenius overestimate produced by a small probe budget yields a \textit{conservative} spectral margin: the CBF-QP compensates for the inflated $\hat{\sigma}_{\max}$ by widening the safety tube, preserving forward invariance at the cost of slightly reduced utility---an acceptable trade-off for safety-critical operation.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figure_13.png}
    \caption{\texorpdfstring{\textbf{Experiment IX: Scalability at $n = 128, 512, 1024$ with Integrated Hutchinson Spectral Margin.}}{Experiment IX: Scalability at n = 128, 512, 1024 with Integrated Hutchinson Spectral Margin.} (a)~Wall-clock time scales predictably with dimension (the $O(n^2)$ cost is due to the dense $Ax$ computation in the figure-generation code; a JVP-based implementation would achieve $O(n)$). (b)~Hutchinson $\tilde{\sigma}_{\max}$ estimates grow with $\sqrt{n}$ as expected for Frobenius-norm overestimation (see Section~3.5.2); these estimates are actively fed into the CBF-QP via Equation~\ref{eq:robust_barrier} at each step. (c)~Minimum barrier values remain strictly positive across all dimensions: 0/60 violations total, with higher minimum barrier values than a non-Hutchinson baseline due to the conservative spectral margin. (d)~CBF activation frequency increases moderately with dimension (reflecting the Hutchinson-inflated tube), confirming that the adaptive margin trades utility for additional safety conservatism as dimensionality grows.}
    \label{fig:scalability}
\end{figure}

\textbf{Results:} Zero safety violations across all 60 trials (20 per dimension). Wall-clock time scales from 0.02\,s ($n{=}128$) to 1.5\,s ($n{=}1024$) per trial; the superlinear growth reflects the $O(n^2)$ dense matrix-vector product in the figure-generation code (a JVP-based implementation would achieve $O(n)$). The Hutchinson $\tilde{\sigma}_{\max}$ estimates exhibit the expected $\sqrt{n}$ Frobenius overestimation (Section~3.5.2); these estimates are integrated into the CBF-QP constraint at each step, tightening the barrier by $\rho(x) = \tilde{\sigma}_{\max} \cdot \Delta t$. CBF activation frequency increases moderately with dimension ($\sim$156 at $n{=}128$ to $\sim$175 at $n{=}1024$ of 300 steps), reflecting the Hutchinson-inflated tube's increased conservatism at higher dimensions---a desirable property for safety-critical operation, as the wider margin absorbs the larger spectral uncertainty inherent in higher-dimensional dynamics.

\subsection{Experiment X: Closed-Form QP vs.\ OSQP Benchmark (Single-Constraint)}
A key claim of this paper is that the single-constraint CBF-QP (Equation~\ref{eq:cbfqp}) admits a closed-form solution computable in $O(n)$ time, circumventing iterative QP solvers. To validate this empirically, we benchmark our closed-form projection against OSQP \cite{osqp2020}, a widely used operator-splitting QP solver, on identical single-constraint barrier-enforcement problems across dimensions $n \in \{64, 128, 256, 512, 1024, 2048\}$. \textbf{Important caveat:} the closed-form solution applies \textit{only} to the single-constraint case ($m = 1$). For systems with multiple simultaneously active CBF constraints, a general QP solver (such as OSQP) is required; the speedup reported here quantifies the advantage of the CHDBO design choice to aggregate multiple constraints into a single barrier via smooth approximation (Section~4.2), not a general superiority over iterative solvers.

\textbf{Protocol:} For each dimension, 1{,}000 QP instances are generated: the state $x$ is sampled near the boundary of the unit ball ($\|x\| \in [0.85, 0.97]$), and the nominal control alternates between aggressive outward pushes (constraint active, 50\%) and conservative inward controls (constraint inactive, 50\%). Both solvers receive identical $(u_{\text{nom}}, \nabla h, \text{rhs})$ triples. OSQP is configured with warm-starting enabled, $\epsilon_{\text{abs}} = \epsilon_{\text{rel}} = 10^{-6}$, and 200 maximum iterations. Wall-clock time is measured via \texttt{time.perf\_counter}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figure_15.png}
    \caption{\texorpdfstring{\textbf{Experiment X: Closed-Form CBF-QP vs.\ OSQP, Single Constraint ($n = 64$--$2048$).}}{Experiment X: Closed-Form CBF-QP vs. OSQP, Single Constraint (n = 64 -- 2048).} (a)~Log-log solve time: the closed-form solution scales as $O(n)$ and remains under 4\,$\mu$s even at $n = 2048$, while OSQP grows to $>$1\,ms. The dashed line shows the 10\,ms real-time budget. (b)~Speedup factor: 61$\times$ at $n = 64$, rising to $\sim$351$\times$ at $n = 2048$, confirming superlinear divergence. This speedup is specific to the single-constraint case; multi-constraint problems require a general QP solver. (c)~Grouped comparison of per-solve times on log scale.}
    \label{fig:qp_benchmark}
\end{figure}

\textbf{Results:} The closed-form QP achieves a \textbf{61$\times$ to 351$\times$ speedup} over OSQP across all tested dimensions. At $n = 2048$, the closed-form solution averages $\sim$3\,$\mu$s per solve versus $>$1{,}000\,$\mu$s for OSQP. The speedup grows with dimension because the closed-form cost is dominated by two $O(n)$ dot products (checking feasibility and computing the Lagrange multiplier), while OSQP's per-iteration cost includes matrix factorization and iterative convergence steps that scale superlinearly in practice. Both solvers produce identical safety decisions on all 6{,}000 test instances. This confirms that the $O(n)$ claim in Section~4.2 yields orders-of-magnitude practical advantage over industry-standard QP solvers for the single-constraint case, validating the real-time feasibility of CHDBO's design choice to aggregate constraints into a single barrier.

\subsection{Experiment XI: Empirical Validation of Theorem~\ref{thm:safe_convergence} (Safe Utility Convergence)}
...
\begin{figure}[htbp]
    ...
    \caption{\texorpdfstring{\textbf{Experiment XI: Empirical Validation of Theorem~\ref{thm:safe_convergence} ($\mathbb{R}^{128}$, Linear Drift).}}{Experiment XI: Empirical Validation of Safe Convergence Theorem (R128, Linear Drift).} (a)~Mean utility $U(x(t))$ across 20 trials converges monotonically toward the constrained optimum $U^* = -0.50$ (dashed). (b)~Barrier values $h(x(t))$ remain strictly positive ($\geq 0$) for all trials and all steps: 0/24{,}000 safety violations. (c)~Norm trajectories $\|x(t)\|$ approach the boundary from below, consistent with the constrained optimum lying on $\partial\mathcal{S}$. (d)~Distance to constrained optimum $\|x(t) - x^*_\mathcal{S}\|$ decreases monotonically, confirming convergence to a single KKT point as predicted by the {\L}ojasiewicz argument.}
    \label{fig:utility_convergence}
\end{figure}

\textbf{Results:} Across 20 trials $\times$ 1{,}200 steps (24{,}000 total state evaluations), the system achieves \textbf{0 safety violations}. The mean utility converges to $U = -0.554$ (vs.\ optimal $U^* = -0.50$), with the gap attributable to the linear drift consuming control authority that would otherwise drive the agent toward the optimum. The mean distance to $x^*_\mathcal{S}$ decreases monotonically to 0.194 at termination, confirming that the trajectory approaches the KKT set predicted by Theorem~\ref{thm:safe_convergence}. These results empirically validate the key claims of Theorem~\ref{thm:safe_convergence}: (1)~forward invariance is maintained throughout ($h(x(t)) \geq 0$), (2)~the utility function is non-decreasing in expectation, and (3)~the trajectory converges toward the constrained optimum rather than oscillating or diverging. The residual gap between achieved and optimal utility quantifies the cost of safety enforcement under adverse drift---precisely the trade-off formalized in Theorem~\ref{thm:safe_convergence}'s Remark on General Control-Affine Dynamics.

\subsection{Experiment XII: MCBC vs.\ Scenario Approach --- Sample Complexity Comparison}
\label{sec:exp_scenario}

To benchmark our Monte Carlo Barrier Certificate against the classical Scenario Approach \cite{campi2008, calafiore2006}, we compare the sample complexity required for equivalent statistical guarantees across dimensions $n = 2$ to $n = 1024$.

\textbf{Setup.} The scenario approach requires $N_{\text{scenario}} \geq \frac{2}{\epsilon}(\ln\frac{1}{\beta} + n)$ samples for a convex program with $n$ decision variables, violation probability $\epsilon$, and confidence $\beta$ \cite{campi2008}. The MCBC requires $N_{\text{MCBC}} = \frac{1}{2\epsilon^2}\ln\frac{2}{\delta}$ samples for verification of a fixed barrier with failure fraction tolerance $\epsilon$ and confidence $\delta$ (Equation~\ref{eq:hoeffding}). We set $\epsilon = 0.01$ and $\delta = \beta = 10^{-6}$ for fair comparison, and evaluate both the sample count and total computational cost (samples $\times$ per-sample cost) across all dimensions.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figure_17.png}
    \caption{\texorpdfstring{\textbf{Experiment XII: MCBC vs.\ Scenario Approach Sample Complexity.}}{Experiment XII: MCBC vs. Scenario Approach Sample Complexity.} (a)~Required samples: MCBC maintains a constant $N \approx 72{,}500$ regardless of dimension, while the scenario approach starts lower ($N \approx 3{,}200$ at $n=2$) but grows linearly as $O(n)$, crossing the MCBC line near $n \approx 350$ and reaching $\sim$208{,}000 at $n = 1024$ ($\approx 2.9\times$ more). (b)~Total computational cost: accounting for the $O(n)$ per-sample cost of barrier evaluation versus the $O(n^2)$ solver cost of the scenario approach, the total gap exceeds three orders of magnitude at $n = 1024$.}
    \label{fig:scenario_comparison}
\end{figure}

\textbf{Results:} The MCBC sample count is $N_{\text{MCBC}} \approx 72{,}500$ for all dimensions tested, confirming the dimension-independent scaling of the Hoeffding bound~\eqref{eq:hoeffding}. The scenario approach requires only $N_{\text{scenario}} \approx 3{,}200$ at $n = 2$---far fewer than MCBC---but grows linearly, crossing the MCBC line near $n \approx 350$ and reaching $N_{\text{scenario}} \approx 208{,}000$ at $n = 1024$ ($2.9\times$ more than MCBC). The critical distinction is that these approaches solve \textit{different problems}: the scenario approach simultaneously \textit{designs} a feasible controller while providing PAC guarantees, whereas the MCBC \textit{verifies} a pre-specified barrier. At low dimensions ($n < 350$), the scenario approach is the more sample-efficient choice; the MCBC advantage emerges precisely in the high-dimensional regime ($n > 350$) most relevant to semantic agent verification. When the barrier is given (as in our framework), the MCBC's $O(1)$ sample complexity delivers substantial computational savings at scale. When the barrier must be learned, the scenario approach's $O(n)$ cost is unavoidable---but this cost is incurred during \textit{design}, not during real-time operation.
...
\subsection{Experiment XVI: Bounded Actuation and CBF-QP Feasibility}
\label{sec:exp_bounded_actuation}

All preceding experiments assume unbounded control authority ($\mathcal{U} = \mathbb{R}^n$). In practice, actuators have finite capacity, and the CBF-QP may become infeasible when the required intervention $\|u^*\|$ exceeds the actuation budget $u_{\max}$. This experiment systematically evaluates the interaction between actuation limits and safety guarantees under nonlinear Lorenz-type dynamics.

\textbf{Setup.} We use a linear barrier $h(x) = w^\top x + b$ in $\mathbb{R}^{128}$ with coupled Lorenz-type dynamics (scaling $1/20$, identical structure to Experiment~VIII). For each actuation budget $u_{\max} \in \{0.05, 0.1, 0.2, 0.5, 1.0, 2.0, 5.0, \infty\}$, we run 30 trials of 500 steps ($\Delta t = 0.001$, RK4 integration) and measure the fraction of trials that maintain $h(x) \geq 0$ throughout. The CBF-QP enforces $w^\top(f(x) + u) \geq -\gamma h(x)$ ($\gamma = 0.5$) with the additional constraint $\|u\| \leq u_{\max}$: if the minimum-norm $u^*$ exceeds the budget, it is rescaled to $u_{\max} \cdot u^* / \|u^*\|$.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figure_19.png}
    \caption{\texorpdfstring{\textbf{Experiment XVI: Bounded Actuation CBF-QP Feasibility ($\mathbb{R}^{128}$, Lorenz Dynamics).}}{Experiment XVI: Bounded Actuation CBF-QP Feasibility (R128, Lorenz Dynamics).} Safety rate vs.\ actuation budget $u_{\max}$: at very low budgets ($u_{\max} = 0.05$), approximately $90\%$ of trials maintain safety; safety improves monotonically with budget, reaching 100\% at $u_{\max} \geq 2.0$. The unbounded case ($u_{\max} = \infty$) achieves 100\% as expected. The smooth transition quantifies the graceful degradation when actuation authority is insufficient.}
    \label{fig:bounded_actuation}
\end{figure}

\textbf{Results.} Safety degrades gracefully as the actuation budget decreases: 100\% at $u_{\max} \geq 2.0$, decreasing to $\sim$90\% at $u_{\max} = 0.05$. Critically, even at the lowest budget, the CBF-QP provides \textit{best-effort} safety rather than catastrophic failure---the clipped intervention still pushes the state toward the safe region, just not always strongly enough. This experiment demonstrates that CHDBO degrades gracefully under actuation constraints, provides empirical guidance for actuation budget selection (practitioners should ensure $u_{\max}$ exceeds the 95th percentile of $\|u^*\|$ under nominal dynamics), and confirms that the closed-form QP naturally extends to bounded-actuation settings via simple norm clipping.

\subsection{Experiment XVII: CHDBO Failure Modes and Limitations}
\label{sec:exp_failure_modes}

To honestly characterize the boundaries of the CHDBO framework, we systematically probe three failure modes: actuation starvation, the drift-vs-control tradeoff, and MCBC calibration.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figure_21.png}
    \caption{\texorpdfstring{\textbf{Experiment XVII: CHDBO Failure Modes.}}{Experiment XVII: CHDBO Failure Modes.} (a)~Actuation starvation: safety rate vs.\ actuation budget with a linear barrier ($n = 50$, Lorenz dynamics). Safety transitions from $0\%$ at $u_{\max} = 0.05$ through $73\%$ at $u_{\max} = 0.15$ to $100\%$ at $u_{\max} \geq 0.30$, revealing a sharp phase transition at $u_{\max} \approx 0.15$. (b)~Drift vs.\ control budget: as the drift magnitude increases (at fixed $u_{\max} = 0.5$), safety drops from 100\% to 0\% when the drift/budget ratio exceeds $\sim$1.0. (c)~MCBC verification honesty: the measured $P_{\text{safe}}$ from MCBC matches the expected value (within 95\% Clopper--Pearson confidence intervals) for all target failure rates from 0.00 to 0.50, confirming that the MCBC does not over-promise when the CBF is genuinely insufficient.}
    \label{fig:failure_modes}
\end{figure}

\textbf{(a) Actuation Starvation.} Using a linear barrier in $\mathbb{R}^{50}$ with Lorenz drift ($\gamma = 0.3$, $\Delta t = 0.001$), we sweep $u_{\max}$ from 0.05 to 1.0. The safety rate exhibits a sharp phase transition: 0\% at $u_{\max} = 0.05$, 73\% at $u_{\max} = 0.15$, and 100\% at $u_{\max} \geq 0.30$. The transition occurs when $u_{\max}$ crosses the typical $\|u^*\|$ required by the CBF-QP under Lorenz dynamics. Below this threshold, the clipped intervention is systematically insufficient, and barrier violations accumulate rapidly.

\textbf{(b) Drift vs.\ Control Budget.} At fixed $u_{\max} = 0.5$, we increase the drift magnitude and observe safety dropping from 100\% to 0\% when drift/$u_{\max}$ exceeds $\sim$1.0. This cleanly demonstrates the fundamental limit: when $\|f(x)\| \gg u_{\max}$, no CBF-QP can maintain safety. The transition ratio provides a practical design rule: \textit{the actuation budget must exceed the expected drift magnitude for reliable safety enforcement}.

\textbf{(c) MCBC Honesty.} We inject known failure probabilities (target fail rates from 0.00 to 0.50) by randomly making the CBF-QP fail at prescribed rates, then verify that the MCBC's measured $P_{\text{safe}}$ matches the expected value. Across all target rates, the measured value falls within the 95\% Clopper--Pearson confidence interval, confirming that the MCBC provides honest estimates even when safety is genuinely compromised.

\textbf{Significance.} This experiment does not demonstrate a strength of CHDBO---it demonstrates its \textit{limitations} honestly. The framework fails when actuation is insufficient, when drift overwhelms control authority, and the MCBC correctly detects these failures rather than hiding them. We view this as a strength of the framework's design: a safety system that reports success even when safety is violated is more dangerous than one that honestly fails.
...
VIII (Lorenz, $n{=}128$) & Safety Rate & 0/50 violations & Lorenz $f(x)$ & CBF-QP, nonlinear \\
 & Mean $\|f(x)\|$ & 11.2 & & Coupled cubic, $1/8$ scale \\
IX (Scale, $n{\leq}1024$) & Safety Rate & 0/60 violations & $\dot{x}=Ax+u$ & $n{=}128,512,1024$ \\
 & Time ($n{=}1024$) & 1.5\,s/trial & & Hutch.\ $k{=}5$ in QP \\
X (QP Benchmark) & Speedup vs.\ OSQP & 61--351$\times$ & $\dot{x}=u$ & Single constraint \\
 & Closed-form time & $\sim$3\,$\mu$s/solve & & $n{=}64$--$2048$ \\
XI (Thm.~\ref{thm:safe_convergence} Valid.) & Safety Rate & 0/24{,}000 evals & $\dot{x}=Ax+u$ & CBF-QP, Thm.~\ref{thm:safe_convergence} \\
 & Utility Convergence & $U \to -0.554$ & & 20 trials, 1200 steps \\
XII (Scenario Comp.) & MCBC Samples & 72{,}544 ($\forall n$) & --- & Hoeffding bound \\
...
XVI (Bounded Actuation) & Safety vs.\ $u_{\max}$ & 90\%--100\% & Lorenz $f(x)$ & Linear barrier \\
 & Unbounded & 100\% & & $n{=}128$ \\
XVII (Failure Modes) & Phase transition & $u_{\max} \approx 0.15$ & Lorenz $f(x)$ & Starvation \\
 & Drift/budget limit & ratio $\approx 1.0$ & & Physics boundary \\
 & MCBC calibration & Within 95\% CI & & Clopper--Pearson \\
\bottomrule
\end{tabular}
\begin{flushleft}
\small \textit{Notes:} Experiments I--IV and VI--VII use single-integrator dynamics ($f(x)=0$); Experiment V introduces linear/bilinear drift; Experiment VIII uses nonlinear chaotic drift (coupled Lorenz-type, $1/8$ scaling, RK4 integration); Experiment IX tests scalability up to $n=1024$; Experiment X benchmarks the single-constraint closed-form QP against OSQP (multi-constraint problems require a general solver); Experiment XI validates Theorem~\ref{thm:safe_convergence}; Experiment XII compares MCBC against the scenario approach (verification-only vs.\ design+verify); Experiment XIII validates the union bound; Experiment XIV validates CHDBO on GPT-2 ($n=768$) with train/test split, perplexity evaluation, and controllability discussion; Experiment XV demonstrates AASV with a learned barrier gradient (no oracle); Experiment XVI quantifies bounded-actuation feasibility; Experiment XVII characterizes failure modes honestly. ``WTA'' = Winner-Take-All gradient decomposition. ``ECBF'' = Exponential CBF for relative degree 2. Computation times measured on a single CPU thread.
\end{flushleft}
\label{tab:results}
\end{table}
...
We explicitly acknowledge that Experiments I--IV employ single-integrator dynamics ($\dot{x} = u$, i.e., $f(x) = 0$, $g(x) = I$). This design choice isolates the geometric mechanisms---orthogonal projection, Monte Carlo barrier certification, and AASV adversarial detection---from the confounding effects of drift dynamics, enabling clean validation of each component independently. Experiment~V introduces linear and double-integrator drift in $\mathbb{R}^{128}$; Experiment~VIII extends to a strongly nonlinear chaotic regime (coupled Lorenz-type attractor with cubic coupling terms and inter-triplet diffusive coupling in a ring topology); Experiment~IX validates scalability to $n = 1024$ with integrated Hutchinson spectral margins; Experiment~XI empirically validates the convergence claim of Theorem~\ref{thm:safe_convergence} under linear drift; Experiment~XII benchmarks sample complexity against the scenario approach; Experiment~XIII quantifies union bound degradation at long horizons; Experiment~XIV extends CHDBO to real, learned, nonlinear dynamics by treating GPT-2's transformer layers as a discrete-time residual dynamical system on $\mathbb{R}^{768}$, with a data-learned SVM barrier and a production toxicity benchmark; Experiment~XV validates that a neural network barrier can replace the structural oracle for AASV spike detection in $\mathbb{R}^{20}$; Experiment~XVI characterizes safety degradation under bounded actuation with Lorenz-type drift; and Experiment~XVII provides an honest failure-mode analysis, identifying the phase transitions where CHDBO's guarantees break down. Together, these experiments confirm that the CBF-QP safety filter maintains forward invariance across linear, bilinear, nonlinear, and learned transformer dynamics, while also characterizing the regimes where safety degrades. Remaining limitations include learned neural ODE dynamics \cite{chen2018} and systems with state-dependent control matrices $g(x) \neq I$.

This limitation does not invalidate the theoretical results: Theorems~\ref{thm:topological_safety} and~\ref{thm:safe_convergence} are stated and proved for the general control-affine system $\dot{x} = f(x) + g(x)u$, and the AASV module's spectral margin (Equation~\ref{eq:robust_barrier}) explicitly accounts for drift volatility via $\rho(x) = \tilde{\sigma}_{\max}(J_f) \cdot d_{\text{step}}$.
...
The trajectory safety guarantee (Proposition~\ref{prop:trajectory_bridge}) degrades linearly with horizon length $T$, becoming vacuous at $T \geq 1/\epsilon$. While periodic re-certification and AASV partially mitigate this (see Remark on Horizon Limitation in Section~3.4), the fundamental linear accumulation of the union bound is a structural limitation of the current approach. For long-horizon deployments ($T > 10^6$ steps), tighter bounds require either supermartingale barrier certificates \cite{santoyo2021, clark2021} or exponential stability arguments not developed in this work.

\subsubsection{Multi-Constraint Aggregation}
The $O(n)$ closed-form CBF-QP solution (Section~4.2) requires aggregation of multiple safety constraints into a single barrier $h(x)$ via smooth approximation (e.g., LogSumExp). This aggregation introduces conservatism: the smooth minimum is always strictly less than the hard minimum, so the effective safe set is a strict subset of $\bigcap_i \mathcal{S}_i$. For systems with many constraints that are simultaneously active, the conservatism can significantly reduce the feasible operating region. Maintaining multiple independent CBF constraints recovers the full safe set but requires a general QP solver at $O(n^3)$ cost per step, negating the closed-form advantage.

\subsubsection{Dimension-Independence Caveats}
While the MCBC sample complexity $N = O(\epsilon^{-2} \ln \delta^{-1})$ is independent of $n$, the per-sample cost is $O(n)$ (dominated by barrier and Lie derivative evaluation), so the total verification cost scales as $O(Nn)$---linear in $n$, not constant. Furthermore, the Lipschitz constant $L_h$ of the barrier function may itself grow with $n$ for certain barrier designs (e.g., $L_h = \sqrt{n}$ for $h(x) = 1 - \|x\|^2/n$), in which case the effective sample complexity required to achieve a fixed safety margin $\varepsilon_s = h_{\min}/L_h$ increases implicitly with dimension. The dimension-independence holds rigorously for barrier functions with bounded $L_h$ (as in our experiments), which is a design choice rather than a universal guarantee.

\subsubsection{Bounded Actuation}
The closed-form CBF-QP (Section~4.2) solves an unconstrained QP; actuation limits $\|u\| \leq u_{\max}$ are enforced by post-hoc norm clipping $u^* \leftarrow u_{\max} \cdot u^*/\|u^*\|$ when $\|u^*\| > u_{\max}$. This preserves direction but may violate the CBF decrease condition when the required correction exceeds the actuation budget. Experiment~XVI quantifies this trade-off: for the Lorenz-type attractor at scale $1/20$, safety degrades from $100\%$ to $\sim$$90\%$ as $u_{\max}$ decreases from $5.0$ to $0.1$. Experiment~XVII identifies the precise phase transition at drift-to-budget ratio $\|f\|/u_{\max} \approx 1.0$: above this threshold, the controller cannot counteract drift, and safety violations become inevitable regardless of barrier design. Practitioners must verify that $u_{\max} > \sup_{x \in \partial\mathcal{S}} \|u^*(x)\|$ or accept graceful degradation.

\subsubsection{Discrete-Time CBF Constraints}
The continuous-time CBF condition $\dot{h}(x) + \gamma h(x) \geq 0$ becomes the discrete-time constraint $h(x_{k+1}) \geq (1-\gamma)h(x_k)$ with $\gamma \in (0, 1]$, requiring $\gamma \leq 1$ to prevent sign reversal of the safety margin. For the linear barrier $h(x) = w^\top x - b$ used in Experiment~XIV, the discrete-time condition is \emph{exact}: $h(x + \Delta x) = h(x) + w^\top \Delta x$ with no higher-order remainder. However, for spherical barriers $h(x) = R^2 - \|x\|^2$, the Taylor expansion yields a quadratic remainder $-\|\Delta x\|^2$ that accumulates per step. In high dimensions ($n \gg 1$), even small perturbations $\|\Delta x\| = O(1/\sqrt{n})$ produce $O(1)$ accumulated error over $O(n)$ steps, potentially violating safety. This is a fundamental advantage of linear (hyperplane) barriers for discrete-time systems: they incur zero discretization error and their Lipschitz constant $L_h = \|w\|$ is independent of dimension.

\section{Conclusion}
The pursuit of autonomous systems that are both demonstrably safe and operationally aggressive has long been framed as a zero-sum trade-off in control theory. To guarantee safety via Hamilton-Jacobi reachability or grid-based verification, one historically had to sacrifice scalability, resigning formal proofs to low-dimensional toy problems. Conversely, to achieve high utility via Reinforcement Learning, one had to sacrifice guarantees, accepting a non-zero probability of catastrophic failure, a phenomenon known as ``mutational meltdown'' or ``Agent Drift'', in exchange for performance.

This paper has introduced Constrained High-Dimensional Barrier Optimization (CHDBO), a unified framework that addresses this dichotomy. By shifting the verification paradigm from deterministic enumeration (which scales exponentially, $O((1/\eta)^n)$) to probabilistic certification (which scales as $O(1)$ in sample complexity for fixed Lipschitz constant $L_h$ and safety margin $\varepsilon_s$), we have circumvented the ``Curse of Dimensionality'' that has plagued safety-critical autonomy for decades \cite{bellman1957}. We emphasize that this probabilistic relaxation trades deterministic certainty for high-confidence statistical bounds---a necessary concession for high-dimensional systems, complemented by the adversarial guarantees of AASV.

Our empirical results confirm seven critical advancements:
\begin{enumerate}
    \item \textbf{Safety is Probabilistically Scalable:} Through Monte Carlo barrier certificates bounded by Lipschitz continuity, we can verify the boundary consistency of $\mathbb{R}^{128}$ manifolds with high statistical confidence ($1-\delta$), bounding the volume fraction of failure modes. Proposition~\ref{prop:trajectory_bridge} formally connects this boundary-level certificate to trajectory-level safety via a union bound.
    \item \textbf{Utility is Asymptotic:} By projecting utility gradients onto the tangent cone of the safe set, we ensure that agents converge to global constrained optima without violating invariant constraints. Experiment~XI provides direct empirical validation of Theorem~\ref{thm:safe_convergence}'s convergence guarantee under linear drift in $\mathbb{R}^{128}$.
    \item \textbf{Safety and Performance are Compatible:} We have demonstrated that high performance and rigorous safety are not mutually exclusive. An agent can operate at the very edge of the safe set $\mathcal{S}$ without ever crossing the line into failure.
    \item \textbf{Black Swans are Huntable (with Structural Gradient Access):} The AASV module transitions safety assurance from passive statistical estimation to active, optimization-based threat hunting, with the formal detection bound of Theorem~\ref{thm:aasv_detection}. Across eight configurations in $\mathbb{R}^{128}$---including orthogonal, antipodal, clustered, and random spike geometries---standard Monte Carlo sampling detected zero injected singularities, while AASV's WTA-accelerated PGD located all spikes (up to 20/20 with $k=60$ restarts) with perfect reproducibility across 10 independent seeds (Experiment~VII). Critically, the WTA gradient decomposition is a \textit{barrier design guideline}: for multi-modal landscapes, practitioners must structure the barrier so that per-component gradients are accessible (e.g., sum-of-Gaussian wells, multi-head neural architectures \cite{dawson2023}); the full gradient $\nabla h$ (oracle or finite-difference) converges to a centroid saddle, achieving 0/3 detection vs.\ 3/3 with WTA (Experiment~VI). However, Experiment~XV demonstrates that a learned neural network barrier can partially recover spike detection (3/3 via $\nabla h_{\text{NN}}$ in $\mathbb{R}^{20}$) without any WTA decomposition. WTA does not discover arbitrary unknown failure modes---it exploits known barrier structure to efficiently verify against injected or anticipated failure geometries. For single-mode barriers---common in operational safety (e.g., collision boundaries)---standard gradients suffice. The framework generalizes to non-trivial drift dynamics including double-integrator systems (Experiment~V), coupled chaotic nonlinear dynamics (Lorenz-type attractor with inter-triplet diffusive coupling in $\mathbb{R}^{128}$, Experiment~VIII, with dynamics rescaled by $1/8$ and RK4 integration at $\Delta t = 0.001$), and scales practically to $n = 1024$ with integrated Hutchinson spectral margins (Experiment~IX). Furthermore, a head-to-head benchmark against the OSQP solver confirms that the single-constraint closed-form CBF-QP achieves 61--351$\times$ speedups across $n = 64$ to $2048$ (Experiment~X), validating the $O(n)$ real-time feasibility claim for the closed-form QP solve itself (noting that current figure-generation code uses $O(n^2)$ dense matrix operations for the dynamics/gradient computations; see the Code vs.\ Theory Scaling limitation).
    \item \textbf{Convergence is Empirically Validated:} Experiment~XI demonstrates that the CBF-QP control law drives the system toward the constrained optimum under adverse linear drift, with 0/24{,}000 safety violations across 20 trials, confirming the joint safety-utility guarantee of Theorem~\ref{thm:safe_convergence}.
    \item \textbf{The Framework Extends to Real Language Models:} Experiment~XIV demonstrates CHDBO on the hidden-state dynamics of GPT-2 ($\mathbb{R}^{768}$), using a linear SVM barrier learned from the Google Civil Comments toxicity dataset with a proper train/test split (400 train, 100 test). The targeted CBF-QP filter achieves 236/250 correct toxic interventions (94.4\%) with a 4.0\% false activation rate on safe content. Output quality is preserved: the median perplexity ratio for toxic prompts is 1.007 (i.e., 0.7\% degradation), confirming that the safety filter does not destroy fluency. MCBC verification with point-specific KNN dynamics confirms $P_{\mathrm{safe}} = 1.0$ under bounded actuation with dimension-independent sample complexity (noting that the KNN surrogate error has not been formally bounded; see Experiment~XIV for discussion), and the Lipschitz constant $L_h = \|w\| = 0.533$ is literally constant across the state space. This is the first demonstration, to our knowledge, of barrier-based safety enforcement on the internal dynamics of a production language model.
    \item \textbf{Failure Modes are Honestly Characterized:} Experiment~XVII provides a systematic failure-mode analysis, identifying three regimes where CHDBO's guarantees degrade: (a)~actuation starvation, where the safety rate drops sharply when the control budget falls below the required correction magnitude; (b)~drift dominance, with a phase transition at drift-to-budget ratio $\|f\|/u_{\max} \approx 1.0$; and (c)~MCBC calibration, where the empirical violation rate closely tracks the theoretical prediction $\epsilon(N) = \sqrt{\ln(2/\delta)/(2N)}$ (Hoeffding). This honest characterization enables practitioners to verify that their operational regime falls within the guaranteed-safe region before deployment.
\end{enumerate}

We conclude that the future of robust autonomy lies in the synthesis of semantic and kinematic stability. While Holographic Invariant Storage (HIS) provides the \textit{invariant core} of the agent, preserving its goals and personality against context drift, CHDBO provides the \textit{geometric constraint}, ensuring its actions remain within the bounds of safety. The AASV module bridges the gap between statistical certification and adversarial robustness, providing bounded-risk guarantees even against failure modes that are invisible to passive sampling. Together, these technologies offer a framework for persistent safe operation at scale, enabling the deployment of autonomous agents capable of operating reliably over extended time horizons without regression.
...
\textit{Companion Proceedings of The 2019 World Wide Web Conference}, 491--500.

\bibitem{alain2017}
Alain, G. \& Bengio, Y. (2017).
``Understanding Intermediate Layers Using Linear Classifier Probes.''
\textit{5th International Conference on Learning Representations (ICLR), Workshop Track}.

\bibitem{bai2019}
Bai, S., Kolter, J. Z., \& Koltun, V. (2019).
``Deep Equilibrium Models.''
\textit{Advances in Neural Information Processing Systems}, 32, 690--701.

\bibitem{meyer2021}
Meyer, R. A., Musco, C., Musco, C., \& Woodruff, D. P. (2021).
``Hutch++: Optimal Stochastic Trace Estimation.''
\textit{Symposium on Simplicity in Algorithms (SOSA)}, 142--155.
...
\bibitem{zou2023}
Zou, A., Phan, L., Chen, S., Campbell, J., Guo, P., Ren, R., Pan, A., Yin, X., Mazeika, M., Dombrowski, A.-K., Goel, S., Li, N., Lin, Z., Forsyth, M., Scherlis, A., Emmons, S., Madry, A., Steinhardt, J., \& Hendrycks, D. (2023).
``Representation Engineering: A Top-Down Approach to AI Transparency.''
\textit{arXiv preprint arXiv:2310.01405}.

\bibitem{turner2023}
Turner, A., Thiergart, L., Udell, G., Leech, G., Mini, U., \& MacDiarmid, M. (2023).
``Activation Addition: Steering Language Models Without Optimization.''
\textit{arXiv preprint arXiv:2308.10248}.

\bibitem{jax2018}
Bradbury, J., Frostig, R., Hawkins, P., Johnson, M. J., Leary, C., Maclaurin, D., Necula, G., Paszke, A., Vander{P}las, J., Wanderman-{M}ilne, S., \& Zhang, Q. (2018).
...
Confidence $\delta$ & $10^{-6}$ & $10^{-4}$ to $10^{-6}$ \\
Failure tolerance $\epsilon$ & $10^{-3}$ & $10^{-2}$ to $10^{-3}$ \\
Sample count $N$ & $\frac{1}{2\epsilon^2}\ln\frac{2}{\delta} \approx 7 \times 10^6$ & $\approx 7 \times 10^4$ \\
Re-certification interval $T_w$ & $10^3$ steps & $10^2$ to $10^3$ steps \\
\bottomrule
\end{tabular}
\caption{Recommended MCBC parameters.}
\end{table}

\textbf{Kinematic recommendation.} For safety-critical physical systems ($n \leq 128$), use strict tolerances ($\epsilon = 10^{-3}$, $\delta = 10^{-6}$) and re-certify every $10^3$ control steps to keep the trajectory-level failure probability below $10^{-2}$ per re-certification window (Experiment~XIII).
