...
The gold standard for deterministic safety verification is H-J reachability analysis \cite{mitchell2005}, which computes the backward reachable set via viscosity solutions of the H-J PDE. While providing the strongest possible guarantees, its $O((1/\eta)^n)$ grid complexity limits practical application to $n \leq 5$. Recent work on decomposition-based \cite{chen2018} and learning-based approximations aims to extend reachability to moderate dimensions, but the fundamental exponential barrier remains for exact computation. Our probabilistic relaxation sidesteps this barrier entirely.
...
In this section, we present a comprehensive evaluation of the Constrained High-Dimensional Barrier Optimization (CHDBO) framework. We position our approach relative to Model Predictive Control (MPC) \cite{camacho2013} and Deep Reinforcement Learning (DRL) \cite{schulman2017}, evaluating CHDBO's safety, utility, and scalability across ten progressively challenging configurations.
...
X (QP Benchmark) & Speedup vs.\ OSQP & 60--324$\times$ & $\dot{x}=u$ & $n{=}64$--$2048$ \\
 & Closed-form time & 1.5--3.8\,$\mu$s/solve & & 1000 solves/dim \\
\bottomrule
\end{tabular}
\begin{flushleft}
\small \textit{Notes:} Experiments I--IV and VI--VII use single-integrator dynamics ($f(x)=0$); Experiment V introduces linear/bilinear drift; Experiment VIII introduces nonlinear chaotic drift (coupled Lorenz-type with nearest-neighbor diffusion); Experiment IX tests scalability up to $n=1024$; Experiment X benchmarks the closed-form QP against OSQP. ``WTA'' = Winner-Take-All gradient decomposition. ``ECBF'' = Exponential CBF for relative degree 2. Computation times measured on a single CPU thread.
\end{flushleft}
\label{tab:results}
\end{table}
