...
# ============================================================================
# Offline region
offline_bg = FancyBboxPatch((0.2, 7.0), 5.6, 2.8,
    boxstyle="round,pad=0.15", facecolor=COL_BG_OFF, edgecolor=COL_OFFLINE,
    linewidth=2, linestyle='--', alpha=0.5)
ax.add_patch(offline_bg)
ax.text(3.0, 9.55, 'OFFLINE / PERIODIC', fontsize=11, fontweight='bold',
...
ax.text(8.0, 6.25, 'ONLINE — PER CONTROL STEP ($\\mathbf{O(n)}$ per step)',
        fontsize=11, fontweight='bold', color='#1a8a4a', ha='center', style='italic')
...
def draw_box(ax, x, y, w, h, label, sublabel, color, text_color='white'):
    ...
    if sublabel:
        ax.text(x + w/2, y + h/2 - 0.25, sublabel, fontsize=8,
                color=text_color, ha='center', va='center', alpha=0.9)
...
ax.text(4.0, 5.2, '$u_{\\mathrm{nom}}$', fontsize=9, ha='center', fontweight='bold')

# CBF-QP → Plant
ax.annotate('', xy=(6.25, 4.2), xytext=(6.25, 2.5), arrowprops=arrow_kw)
ax.text(5.8, 3.35, '$u^*$', fontsize=10, ha='center', fontweight='bold', color=COL_SAFETY)

# CBF-QP → AASV (predict)
ax.annotate('', xy=(9.0, 4.95), xytext=(8.0, 4.95), arrowprops=arrow_kw)
ax.text(8.5, 5.25, '$x_{\\mathrm{pred}}$', fontsize=9, ha='center', fontweight='bold')

# AASV ↔ Anti-Memory
ax.annotate('', xy=(13.0, 5.2), xytext=(12.5, 5.2), arrowprops=arrow_kw)
ax.annotate('', xy=(12.5, 4.5), xytext=(13.0, 4.5),
            arrowprops=dict(arrowstyle='->', color=COL_MEMORY, lw=2, mutation_scale=18))
ax.text(12.75, 5.5, 'store', fontsize=8, ha='center', color='#2c3e50')
ax.text(12.75, 4.15, 'repel', fontsize=8, ha='center', color=COL_MEMORY)
...
ax.text(11.6, 3.35, 'violation\ndetected', fontsize=8, ha='center',
...
ax.text(8.0, 3.85, 'safe ✓', fontsize=9, ha='center',
...
ax.text(2.3, 2.9, '$x(t)$', fontsize=9, ha='center', color=COL_AGENT, fontweight='bold')

# Spectral → AASV
ax.annotate('', xy=(10.75, 5.7), xytext=(10.75, 7.5),
            arrowprops=dict(arrowstyle='->', color='#8e44ad', lw=1.5,
                            mutation_scale=15))
ax.text(11.6, 6.6, '$\\delta(x)$', fontsize=10, ha='center',
...
ax.annotate('', xy=(13.5, 8.5), xytext=(5.8, 8.5),
            arrowprops=dict(arrowstyle='->', color=COL_OFFLINE, lw=1.5,
                            mutation_scale=15))

# Spectral → Certificate
ax.annotate('', xy=(13.5, 8.2), xytext=(13.5, 8.2),
            arrowprops=dict(arrowstyle='->', color='#8e44ad', lw=1.5,
                            mutation_scale=15))
...
ax.text(1.2, 0.3, 'Time $t$: Execute $u^*_t$   |   Hunt $x_{\\mathrm{pred}}(t{+}1)$   |   Certify or Reject',
        fontsize=10, color='#2c3e50', ha='left', style='italic',
